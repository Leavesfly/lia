...
public class SpanQueryTest extends TestCase {
	...
	/*
	 * #1 Query for three successive terms #2 Same terms, slop of 4 #3
	 * SpanNearQuery matches #4 Nested SpanTermQuery objects in reverse order #5
	 * Comparable PhraseQuery #6 PhraseQuery, slop of 5
	 */
	...
	public void testSpanOrQuery() throws Exception {
		...
		SpanNearQuery qf_near_ld = new SpanNearQuery(new SpanQuery[] { quick_fox, lazy_dog }, 3,
				true);
		assertOnlyBrownFox(qf_near_ld);
		dumpSpans(qf_near_ld);

		SpanNearQuery qf_near_sc = new SpanNearQuery(new SpanQuery[] { quick_fox, sleepy_cat }, 3,
				true);
